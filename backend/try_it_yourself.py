...
This still applies when the named person is a historically controversial figure or one
associated with hatred or atrocities (a dictator, a war criminal) - the test is what the
MESSAGE ITSELF says, never who is named. Example: "can I measure Hitler's writing desk"
is NOT a SAFETY case - the message expresses no hatred, degradation, or violence of its
own, it's an ordinary furniture question, so it gets the same plain "yes" as the Elon
Musk example (a desk is ordinary furniture) if Rule 8/8B/8C's requirements are met. Do
not let the historical weight of a name substitute for actually checking whether the
message itself is hateful, degrading, sexual, or violent.
...
# what followed - production testing found a customer replying to a genuine ticket offer
# with "Alright can i get a printout of a maker from a local shop" (a brand new, unrelated
# question) got read as a plain "yes" and silently raised a ticket, since only the first
# word was ever checked. Fixed the same way is_bare_negation() already worked: match the
# ENTIRE stripped message against a fixed set of short affirmative phrases, not just its
# first word - a real confirmation is short by nature, so anything longer or carrying its
# own question falls through to be treated as a new message instead of a confirmation.
AFFIRMATIVE_PHRASES = (
...
# Found via manual testing: messages dressed up as "can Geometra measure X" - a slur, a
# degrading/vulgar term about a person, graphic violence involving corpses, sexual content
# about a named real person - were being treated as legitimate-but-unanswerable product
# questions and offered a support ticket, which is a serious safety failure, not a UX
# quirk. This is checked before EVERYTHING else, including greeting/gratitude, since it's
...
_EXCLUDED_CATEGORIES = (
    ...
    (
        ...
        # (bags, currency, luggage) already were. Same gap found for "backpack".
        re.compile(
...
def process_turn(query, history, awaiting):
    ...
    # See is_print_shop_location_question() - the business's position on naming
    # shops is fixed and already written down, so it is stated rather than
    # re-derived per turn.
    if is_print_shop_location_question(query):
        return PRINT_SHOP_MESSAGE, None
    ...
    # See is_bare_ticket_request() - Rule 2C's own instruction, applied here
    # because Pass 2 applied it inconsistently and sometimes answered with the
    # support email instead. Guarded on awaiting so it cannot hijack a flow
    # already in progress.
    if awaiting is None and is_bare_ticket_request(query):
        return TICKET_NEEDS_DETAIL_MESSAGE, "clarification"
